...
class Model:
    def __init__(self, data_path):
        ...
        try:
            self._processed_new = io.read_file(path=os.path.join(
                self._datapath, 'processed/new_dataset.csv'), datetime_cols=['start_time', 'end_time'])
        except FileNotFoundError:
            print(
    ...
    def train_direction_uni(self):
        ...
        trips_direction = self._processed
        trips_direction["start_time"] = pd.to_datetime(trips_direction["start_time"])
        ...
        for index, row in trips_direction.iterrows():
            ...
            dist_to_uni = self.distanceToUni(row['start_lng'],row['start_lat'],row['end_lng'],row['end_lat'])
        ...
        X_uni = trips_direction[['start_lng','start_lat','humidity_2m','dew_point_2m','max_mean_m/s','hour']]
        ...
        rf_uni = RandomForestClassifier(criterion='entropy', class_weight='balanced_subsample', n_estimators=256, max_depth=7)
    ...
    def train_direction_main_station(self):
        ...
        for index, row in trips_direction.iterrows():
            ...
            dist_to_main_station = self.distanceToMainStation(row['start_lng'],row['start_lat'],row['end_lng'],row['end_lat'])
        ...
        # Initialize independent and target variables
        X_main_station = trips_direction[['start_lng','start_plz','humidity_2m','dew_point_2m','max_m/s','hour']]
        y_main_station = trips_direction['to_main_station_bool']

        print('Training model...')
        
        # Train RandomForestClassifier with optimzed hyperparameters through grid search (see notebook #8)
        rf_main_station = RandomForestClassifier(criterion='entropy', class_weight='balanced_subsample', n_estimators=256, max_depth=7)
    ...
    def train_demand(self, resolution):
        ...
        trips_demand = self._processed

        for col in ['start_time']:
            trips_demand['month'] = pd.DatetimeIndex(trips_demand['start_time']).month
            trips_demand['booking_date'] = trips_demand.start_time.dt.date
            trips_demand['weekdays'] = pd.DatetimeIndex(trips_demand['start_time']).weekday
        ...
        trips_demand = self._setDataset(trips_demand, resolution, "start_time", {"number_bookings":"count", "month":"mean",  "hour":"mean", "temp_2m":"mean", "min":"mean"})
        ...
        poly_features = PolynomialFeatures(degree=switch.get(resolution), include_bias=False)
    ...
    def predict_duration(self):
        ...
        trips_duration = self._processed_new
    ...
    def predict_direction_uni(self):
        ...
        # Initialize independent and target variables
        X = trips_direction[['start_lng','start_lat','humidity_2m','dew_point_2m','max_mean_m/s','hour']]
    ...
    def predict_direction_main_station(self):
        ...
        # Initialize independent and target variables
        X = trips_direction[['start_lng','start_plz','humidity_2m','dew_point_2m','max_m/s','hour']]
            
        # Load prediction model
        model = io.read_model("model_direction_main_station")
        trips_direction['prediction_to_main_station'] = model.predict(X)

        io.save_df(trips_direction, 'direction_prediction_main_station')
        print('Saved prediction.')


    def predict_demand(self, resolution):

        trips_demand = self._processed_new
        ...
        io.save_df(trips_demand, 'duration_prediction_' + resolution)
